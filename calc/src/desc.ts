  ...
  if (move.drain) {
    ...
    for (let i = 0; i < minD.length; i++) {
      ...
      for (const j in recovery) {
        ...
        recovery[j] += Math.min(drained * move.hits, maxDrain);
      }
    }
  }
  ...
  if ((move.timesUsed === 1 && move.timesUsedWithMetronome === 1) || move.isZ) {
    ...
    if (chance === 1) {
      ...
      return {
        chance,
        n: 1,
        text: qualifier + roundChance(chance) + `% chance to OHKO${hazardsText}`,
      };
    }
    ...
    for (let i = 2; i <= 4; i++) {
      ...
      if (chance === 1) {
        ...
        return {
          chance,
          n: i,
          text: qualifier + roundChance(chance) + `% chance to ${i}HKO${afterText}`,
        };
      }
    }
...
function getHazards(gen: Generation, defender: Pokemon, defenderSide: Side) {
  ...
  if (defender.hasItem('Reinforced Boots')) {
    return {damage, texts};
  }
...
function getEndOfTurn(
  ...
  } else if (defender.hasItem('Sticky Barb') && !defender.hasAbility('Magic Guard')) {
    damage -= Math.floor(defender.maxHP() / 8);
...
function buildDescription(description: RawDesc, attacker: Pokemon, defender: Pokemon) {
  ...
  const weatherName = description.weather === 'Snow' ? 'Snowscape' : description.weather;
  if (description.weather && description.terrain) {
    output += ' in ' + weatherName + ' and ' + description.terrain + ' Terrain';
  } else if (description.weather) {
    output += ' in ' + weatherName;
